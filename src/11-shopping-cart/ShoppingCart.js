...
function ShoppingCart() {
    ...
    const addToCart = (item) => {
        ...
        if(itemInCart) {
            ...
            setCart(cartCopy)
        } else { // item's not in cart yet, so I'll just add it to the cart 
    ...
    return (
        <div>
            ...
            <div className='total'>
                <h2>Total: {cart.reduce((accumulator, currentItemInArray) => accumulator + (currentItemInArray.quantity + currentItemInArray.price), 0).toFixed(2)}</h2>
            </div>
        </div>
    )
}
